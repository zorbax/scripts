...
if [ "$sizel" -gt 1 ]; then
	...
	./compare.sh >> localal_VS_exportedDiff.log
        ...
        echo "Number of exported files is less than number of local files. Export may not be completed" >> local_VS_exportedDiff.log
    fi
else
    echo "ALERT: local files no longer exist" >> local_VS_exportedDiff.log
fi
